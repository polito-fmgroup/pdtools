...
#if PDT_INTERFACE
  struct { void (*pdt_learn123)(void *, int, int, int); void *auxp; } pdt_cbs;
#endif
} Cbs;
...
static int lgladdcls (LGL * lgl, int red, int origlue, int force) {
  ...
  if (size == 1) {
    if (lgl->cbs && lgl->cbs->pdt_cbs.pdt_learn123)
      lgl->cbs->pdt_cbs.pdt_learn123 (lgl->cbs->pdt_cbs.auxp,
                                      lglexport(lgl,lit),0,0);
    assert (lglval (lgl, lit) >= 0);
...
void lglstats (LGL * lgl) {
  ...
  if (lgl->opts->verbose.val==1) goto SHORT;
  if (!lgl->opts->verbose.val) goto SUPERSHORT;
...
SUPERSHORT:
  ...
  lglprs (lgl, "LGL: %ld conflicts - %ld decisions - %.2E propagations", 
	  (LGLL) s->confs, (LGLL) s->decisions, (float)(LGLL) p);
  return;
}
...
// G.Cabodi - Nov 2019
void *lglPdtGetInfo(LGL *lgl, char *info, void *aux) {
...
void lglsetpdtlearn (LGL * lgl, void (*pdt_learn123)(void *, int, int, int), void *auxp) {
  REQINITNOTFORKED ();
  lglinitcbs (lgl);
  lgl->cbs->pdt_cbs.pdt_learn123 = pdt_learn123;
  lgl->cbs->pdt_cbs.auxp = auxp;
}

void lglclearpdtlearn (LGL * lgl) {
  REQINITNOTFORKED ();
  if (lgl->cbs==NULL) return;
  lgl->cbs->pdt_cbs.pdt_learn123 = NULL;
  lgl->cbs->pdt_cbs.auxp = NULL;
}

int lglpdtextlit (LGL * lgl, int ilit) {
  return lglexport (lgl, ilit);
}
