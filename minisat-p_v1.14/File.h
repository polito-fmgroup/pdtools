...
#define CHECK_FILE_ERROR 1 // GC 2021 - check file error on writing and wait
...
public:
   ...
   ~File(void) {
    ...
    int putCharQ(int chr) {         // Quick version with minimal overhead -- don't call this in the wrong mode!
        ...
        if (pos == File_BufSize) 
#if !CHECK_FILE_ERROR
          write(fd, buf, File_BufSize), pos = 0;
#else
        {
          int nw, nw_i;
          (nw = write(fd, buf, File_BufSize)), pos = 0;
          while (nw < File_BufSize) {
            sleep(30);
            nw_i = write(fd, buf+nw, File_BufSize-nw);
            if (nw_i > 0) nw += nw_i;
          }
        }
#endif
    ...
    int64 tell(void);
    void  trav_seek(int64 pos);
    int64 trav_tell(void);
