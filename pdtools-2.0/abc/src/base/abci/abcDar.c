/**CFile****************************************************************
  ...
  Revision    [$Id: abcDar.c,v 1.1.1.1 2021/09/11 09:57:46 cabodi Exp $]
...
***********************************************************************/
int Abc_NtkDarSeqSim3( Abc_Ntk_t * pNtk, int nFrames, int nWords, int nBinSize, int nRounds, int nRestart, int nRandSeed, int nCheckedOutputs, int TimeOut, int TimeOutGap, int fSolveAll, int fVerbose, int fNotVerbose )
{
    Aig_Man_t * pMan;
    int status, RetValue = -1;
//    clock_t clk = clock();
    ...
    pMan = Abc_NtkToDar( pNtk, 0, 1 );
    if ( Ssw_RarSimulate( pMan, nFrames, nWords, nBinSize, nRounds, nRestart, nRandSeed, nCheckedOutputs, TimeOut, TimeOutGap, fSolveAll, fVerbose, fNotVerbose ) == 0 )
    { 
        if ( pMan->pSeqModel )
        {
//            Abc_Print( 1, "Simulation of %d frames with %d words asserted output %d in frame %d. ", 
