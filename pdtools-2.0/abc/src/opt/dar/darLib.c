/**CFile****************************************************************
  ...
  Revision    [$Id: darLib.c,v 1.1.1.1 2021/09/11 09:57:46 cabodi Exp $]
...
void Dar_LibEval( Dar_Man_t * p, Aig_Obj_t * pRoot, Dar_Cut_t * pCut, int Required, int * pnMffcSize )
{
    ...
    for ( Out = 0; Out < s_DarLib->nSubgr0[Class]; Out++ )
    {
        ...
        p->ClassBest  = Class;
        if (p->LevelBest > Required) {
	  printf("PLEVEL BEST: %d - Req: %d - Num\n", 
		 p->LevelBest, Required, pObj->Num);
	  //	  assert(p->LevelBest <= Required); // GpC: removed
	  p->GainBest   = -2;
	  Vec_PtrClear( p->vLeavesBest );
	  break;
	}
