/**CFile****************************************************************
  ...
  Revision    [$Id: cecCorr.c,v 1.1.1.1 2021/09/11 09:57:47 cabodi Exp $]
...
{  
    ...
    clock_t clk0 = clock();
    float cpuT, timeLimit = pPars->timeLimit;
    ...
    while ( fChanges )
    {
        ...
        if ( timeLimit > 0 )
        {
	  cpuT = (float)(clock()-clk0)/(float)(CLOCKS_PER_SEC);
	  if (cpuT > timeLimit) {
            Abc_Print( 1, "Time limit reached &scorr: %f.\n", cpuT);
	    pPars->nStepsMax = -2;
            Gia_ManStop( pSrm );
            Vec_IntFree( vOutputs );
            break;
...
{  
    ...
    clock_t clk2, clk = clock(), clk0 = clk;
    float cpuT, timeLimit = pPars->timeLimit;
    ...
    if ( pPars->nStepsMax < -1 )
    {
        Abc_Print( 1, "Stopped signal correspondence after partial BMC.\n" );
        Cec_ManSimStop( pSim );
        return -1;
    }
