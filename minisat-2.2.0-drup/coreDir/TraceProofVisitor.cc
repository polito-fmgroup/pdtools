      ...
      if (mark[i] || resNodes[i].isCareNode()) {
        if (code_i == proof_resA) {
          ...
          if (resNodes[i].isCareNode())
            resNodes[i].setCareNode(false);
        }
        else if (code_i == proof_resB) {
  ...
  static void proofResolveClauses(vec<Lit>& main,
    ...
    for (j=0; j<s; j++) {
      ...
      if (x != var_Undef && v_j == x) {
        ...
        assert(mark[v_j] == l_Undef || mark[v_j]==(sign(l)?l_False:l_True));
        mark[v_j] = l_Undef;
        continue;
      } 
      if (mark[v_j]!=l_Undef) {
...
#pragma GCC pop_options
  ...
  static void pushCareClause(
    ...
    vec<Lit> careClause;
    static int filterWithScareAssign=0;
    ...
    for (int i=i0; i<=i1; i++) {
      ...
      for (int ii=0; ii<resolvents.size(); ii++) {
        Lit l=resolvents[ii];
        Var v = var(l);
        if (!filterWithScareAssign && mark[v]!=l_Undef)
          careClause.push(l);
        mark[v] = l_Undef;
      }
    }
  ...
  static int proofNodeRecyclePivotsReduction(ProofPdt& proofPdt,
    ...
    if (Scare) {
      assert (decLevel == Scare->currDecisionLevel());
    }
